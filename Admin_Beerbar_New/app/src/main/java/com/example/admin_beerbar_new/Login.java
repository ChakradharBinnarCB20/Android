...
import com.example.admin_beerbar_new.Class.ConnectionClass;
      ...
      /*  if (sessionManager.isLoggedIn()) {
        ...
        btn_login.setOnClickListener(new View.OnClickListener() {
            ...
            if (con == null) {
                ...
                String query = "select APK_GROUP_CODE,DEVICE_ID,MOBILE_NO,PASS_WORD from APKLOGIN where MOBILE_NO='"+edit_login_email.getText().toString()+"' and PASS_WORD='"+edit_login_pass.getText().toString()+"' and APK_GROUP_CODE in(select APK_GROUP_CODE from apkgroupmast where apk_type=2)";
                ...
                if(device_id.equals(""))
                {
                    PreparedStatement ps1 = con.prepareStatement("UPDATE APKLOGIN SET DEVICE_ID='" + m_androidId + "' WHERE MOBILE_NO='"+edit_login_email.getText().toString()+"' AND APK_GROUP_CODE='"+group_code+"' and APK_GROUP_CODE in(select APK_GROUP_CODE from apkgroupmast where apk_type=2)");
                    ps1.executeUpdate();
                    new login().execute();
                }
                else
