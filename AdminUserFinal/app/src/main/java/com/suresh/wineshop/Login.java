    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        btn_login.setOnClickListener(new View.OnClickListener() {
            ...
            if (con == null) {
               ...
               // String query = "select APK_GROUP_CODE,DEVICE_ID,MOBILE_NO,PASS_WORD from APKLOGIN where MOBILE_NO='"+edit_login_email.getText().toString()+"'  AND MOBILE_NO NOT IN(SELECT MOBILE_NO from pi_mobile)";
                String query = "select APK_GROUP_CODE,DEVICE_ID,MOBILE_NO,PASS_WORD from APKLOGIN where MOBILE_NO='"+edit_login_email.getText().toString()+"' and APK_GROUP_CODE in(select APK_GROUP_CODE from apkgroupmast where apk_type=1)";
                ...
                if(device_id.equals(""))
                {
                    PreparedStatement ps1 = con.prepareStatement("UPDATE APKLOGIN SET DEVICE_ID='" + m_androidId + "' WHERE MOBILE_NO='"+edit_login_email.getText().toString()+"' AND APK_GROUP_CODE='"+group_code+"' and APK_GROUP_CODE in(select APK_GROUP_CODE from apkgroupmast where apk_type=1)");
                    ps1.executeUpdate();
                    new login().execute();
                }
                else
                ...
                if (con == null) {
                 ...
                 //  String query = "select APK_GROUP_CODE, USER_NAME,MOBILE_NO,PASS_WORD from APKLOGIN where MOBILE_NO='"+lemail+"' and PASS_WORD='"+lpass+"'";
                   // String query = "select APK_GROUP_CODE, USER_NAME,MOBILE_NO,PASS_WORD from APKLOGIN where MOBILE_NO='"+lemail+"' and PASS_WORD='"+lpass+"' and DEVICE_ID='"+m_androidId+"'";
                    String query = "select APK_TYPE, APKLOGIN.APK_GROUP_CODE, USER_NAME,MOBILE_NO,PASS_WORD from APKLOGIN,APKGROUPMAST where APKGROUPMAST.APK_GROUP_CODE=APKLOGIN.APK_GROUP_CODE and MOBILE_NO='"+lemail+"' and PASS_WORD='"+lpass+"' and DEVICE_ID='"+m_androidId+"' and APK_TYPE =1";
                    Statement stmt = con.createStatement();
                    ResultSet rs = stmt.executeQuery(query);
                    if(rs.next())
                    {
